...
def python_function(msg):
    try:
        ...
        raise RuntimeError('Please install the RDFLib and sparqlwrapper Python modules!')

    sparql = SPARQLWrapper("""` + indentLines(config.prefix, 4) + `""")
    sparql.setQuery("""` + indentLines(config.func, 4) + `""")
    sparql.setReturnFormat(JSON)
    results = sparql.query().convert()
    msg['payload'] = results["results"]["bindings"]
    return msg
...
`,
    ...
    RED.nodes.registerType('sparql', PythonFunction);
};
